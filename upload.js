...
async function checkEndpointHealth(endpoint, timeoutMs = 3000) {
    try {
        ...
        try {
            const resp = await fetch(healthUrl, { method: 'GET', mode: 'cors', signal: controller.signal });
            return resp && resp.ok;
        } finally {
...
async function uploadFiles(files, onProgress) {
    ...
    for (let index = 0; index < API_URLS.length; index++) {
        ...
        let reachable = true;
        try {
            reachable = await checkEndpointHealth(endpoint, 3000);
        } catch (_e) {
            reachable = false;
        }

        if (!reachable && !isLastEndpoint) {
            lastAttempt = { status: 0, endpoint, body: { success: false, error: "NETWORK_ERROR" } };
            continue;
        }
